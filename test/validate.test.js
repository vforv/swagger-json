...
describe("swaggerDocJson", () => {
  describe("adding a new GET route", () => {
    ...
    it("should map query parameters to parameters", () => {
      const queryParams = userPath.get.parameters.filter(
        (x) => x.in === "query"
      );
    ...
    it("should transform express path params to swagger path params", async () => {
      ...
      const { paths } = swaggerDocJson.addNewRoute(
        schema,
        "/user/:userId/auth_token",
        "post"
      );
  ...
  describe("adding a new POST route", () => {
    ...
    const { paths, definitions } = swaggerDocJson.addNewRoute(
      schema,
    ...
    it("should map params to path parameters", async () => {
      const pathParams = postPath.post.parameters.filter(
        (x) => x.in === "path"
      );
    ...
    it("should map body params to body parameters", async () => {
      const bodyParams = postPath.post.parameters.filter(
        (x) => x.in === "body"
      );
    ...
    it("should be able to map headers", async () => {
      const headerParams = postPath.post.parameters.filter(
        (x) => x.in === "header"
      );
    ...
    it("should create a default model name if one was not provided", async () => {
      ...
      expect(paths["/create-person"].post.parameters[0].schema.$ref).to.match(
        /\#\/definitions\/\d+/
      );

      const ref =
        paths["/create-person"].post.parameters[0].schema.$ref.split("/");
  ...
  describe("describing an output model", () => {
    const schema = {
      ...
      response: joi
        .object({ name: joi.string(), content: joi.string() })
    ...
    it("should create a default model name if one was not provided", async () => {
      ...
      const { paths, definitions } = swaggerDocJson.addNewRoute(
        schema,
        "/runningOutOfPathNames",
        "get"
      );
      const path = paths["/runningOutOfPathNames"];

      expect(path.get.responses[200].schema.$ref).to.match(
        /\#\/definitions\/\d+/
      );

      const ref =
        paths["/runningOutOfPathNames"].get.responses[200].schema.$ref.split(
          "/"
        );
      const modelName = ref[ref.length - 1];
  ...
  describe("#createJsonDoc", () => {
    it("should be able to be called at the very end", async () => {
      ...
      const addNewRouteOutput = swaggerDocJson.addNewRoute(
        schema,
        "/swagger/examples",
        "get"
      );
      const createJsonDocOutput = swaggerDocJson.createJsonDoc(
        info,
        "http://localhost:4000",
        "/"
      );
